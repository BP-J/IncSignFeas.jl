...
Modifies info and prepare the recursive call in the nonhomogeneous case. 
...
"""
function isf_first_nH!(Vt::Matrix, info::Info, options::Options, values::Values)
    ...
    if options.sv >= 3
        ...
        if options.wechelon
            if options.rational
                isf_allsv_nH_Wechelon_r!(Vt, perm, info, options, values)
            else
                if r < n            # here the rank != the dimension, so it is better to look at the matrix R to avoid numerical issues
                    isf_allsv_nH_Wechelon!([R[1:r, invperm(perm)]; Vt[n+1,:]], perm, info, options, values)
                else
                    isf_allsv_nH_Wechelon!(Vt, perm, info, options, values)
                end
            end
        else
            if options.rational
                isf_allsv_nH_r!(Vt, perm, info, options, values)
            else
                if r < n            # here the rank != the dimension, so it is better to look at the matrix R to avoid numerical issues
                    isf_allsv_nH!([R[1:r, invperm(perm)]; Vt[n+1,:]], perm, info, options, values)
                else
    ...
    elseif options.sv > 0
        ...
        if p > r # p = r means there is nothing to do
            ...
            if r < n 
                isf_somesv_nH!([R[1:r, invperm(perm)] ; Vt[n+1,:]], perm, info, options)
            else
                isf_somesv_nH!(Vt, perm, info, options)
            end
            info.cput_sv += tok()
        end
    end
    ...
    if options.sv == 4  # specific case
        ...
        # retrieval of a common point (nH only)
        # the chosen point is the minimal norm point belonging to the affine subspace
        x_start = (R[:,1:r]' * R[:,1:r]) \ Vt[n+1,perm[1:r]]
        x_start = Q * R[:,1:r] * x_start    # the appropriate value
    ...
    while continuation
        ...
        if r < p
            ...
            if options.withd 
                ...
                isf_rec_nod!(Vt, svec, perm, type, info, options, values)
            end
            info.flag > 0 && return
        else
            info.ns += 1

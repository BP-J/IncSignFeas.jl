### 
# ns, info = brute_force_H(V, options)
...
# Symmetric version of the code, for matrices V = [v1, ..., vp]
...
function brute_force_H(V, options)
    ...
    if options.verb >= 1
        if options.fout == 1
            println("Number of sign vectors = $(2*ns)")
            # time shenanigans
        else
            open(options.fout, "a") do f
                write(f, "\nNumber of sign vectors = $(2*ns)")
            end
        end
    end

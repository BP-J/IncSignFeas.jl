...
"""
`isf_first_H!(V, info, options, values)`
Modifies info and prepare the recursive call in the homogeneous case. 
...
"""
function isf_first_H!(V::Matrix, info::Info, options::Options, values::Values)

    p0 = size(V, 2)
    if options.rational
        V, colsel, colout = isf_noncolin_r!(V)
    else
        V, colsel, colout = isf_noncolin!(V, options)
    end
    ...
    F = qr(V, ColumnNorm()) # the second argument is the default (?) argument so there is a permutation
    Q = F.Q
    R = F.R
    perm = F.p
    ...
    if options.sv >= 3      # 1 means only a few from QR, 2 means these few + some from dual variables of infeasible LOPs along the way
        ...
        if options.wechelon
            if options.rational
                isf_allsv_H_Wechelon_r!(V, perm, info, options, values)
            else
                if r < n            # here the rank != the dimension, so it is better to look at the matrix R to avoid numerical issues
                    isf_allsv_H_Wechelon!(R[1:r, invperm(perm)], perm, info, options, values)
                else 
            ...
            if options.rational
                isf_allsv_H_r!(V, perm, info, options, values)
            else
                if r < n            # here the rank != the dimension, so it is better to look at the matrix R to avoid numerical issues
                    isf_allsv_H!(R[1:r, invperm(perm)], perm, info, options, values)
                else
                    isf_allsv_H!(V, perm, info, options, values)
                end
            end
        end
        info.cput_sv += tok()
    ...
    elseif options.sv > 0
        ...
        if p > r # p = r means there is nothing to do
            ...
            if r < n 
                isf_somesv_H!(R[1:r, invperm(perm)], perm, info, options)
            else
    ...
    if options.sv == 4  # specific case
        ...
        svec = ones(Int,r)          # initial sign vector starting the process
        Rinv = inv(R[1:r, 1:r])'    # used to compute the starting directions
    ...
    while continuation
        ...
        if r < p 
            ...
            else                # algorithm 7 := AD4, only stem vectors
...
#-------------------------------------------------------------------------------------------------------------------------------
    ...
    if p0 != p        
        ...
        if options.sv > 0
            ...
            # the bloc for the pairwise ones
            ...
            info.stems_sym = [info.stems_sym Temp]
            info.stem_sizes_sym = vec(sum(abs.(info.stems_sym), dims=1))            # the sum returns a 1 x [size] matrix, not a vector

            # nothing for the asymmetric ones, just putting it at the right size
            info.stems_asym = Matrix(zeros(p0, 0))
        end
    end

...
Modifies info by calling `isf_allsv_from_indices_H!` on subsets of [1:p].
...
More precisions are found in `isf_allsv_from_indices_H.jl`.
The 'asymmetric' (nH) and 'compact' (HnH) versions are in their respective 
functions (files) `isf_allsv_nH!(.jl)`, `isf_allsv_HnH!(.jl)`.

The computations can be done differently (and often slightly faster), see
`isf_allsv_H_Wechelon(_rec).jl`.
"""
function isf_allsv_H!(V::Matrix, perm::Vector{Int64}, info::Info, options::Options, values::Values)

    p = size(V,2)
    if p < 3
        println("isf_allsv_H!: function is not made for small values of p = $(p)")
...
# ----------------------------------------------------------------------------------------------------
    ...
    # since every subset of 3 <= size <= r+1 is considered, the first one is the one with vectors 1,2,3
    bvec = zeros(Int, p)
    bvec[1:3] = [1,1,1]                                                     # this one
    p_bvec = 3                                                              # binary tree pointer
    ...
    if options.rational                                                     # exact (rational) computations; test before to avoid doing it every time
        while p_bvec > 0                                                    # the loop ends when the pointer is zero
            newstem = false
            if 2 < sum(bvec) < r+2                                          # number of vectors considered ∈ sizes considered
                cols = findall(!iszero, bvec)
                newstem = isf_allsv_from_indices_H_r!(V, cols, info)        # verification of the subset
            end
        ...
        while p_bvec > 0                                                    # the loop ends when the pointer is zero
            ...
            if 2 < sum(bvec) < r+2                                          # number of vectors considered ∈ sizes considered
                cols = findall(!iszero, bvec)
                newstem = isf_allsv_from_indices_H!(V, cols, info, options) # verification of the subset
            end
    ...
    ## now everything has been checked, purge info.stems of its duplicates
    ...
    # nothing to do for the asym part, it is not used
